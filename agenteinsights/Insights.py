...
incluindo carregamento de dados, verificação de alterações, análises de ML,
cruzamento de dados e geração de relatórios.
"""
...
def main() -> None:
    ...
    try:
        ...
        try:
            dados_cruzados = cruzar_dados(dados_atuais, resultados_ml)
            print_status("Cruzamento de dados concluído!", "success")
        except Exception as e:
        ...
        # Gerar relatório
        print_status("Gerando relatório final...", "info")
        try:
            nome_relatorio = f"relatorios/relatorio_{data_hora}.docx"
            gerar_docx(dados_cruzados, nome_relatorio)
            print_status(f"Relatório gerado com sucesso: {nome_relatorio}", "success")
        except Exception as e:
